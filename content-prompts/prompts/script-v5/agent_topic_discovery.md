> **App Connection Note (v5)**: LATER phases push to the Documentary Studio app
> (research → Research tab, script → Script tab, sources → Sources tab, tasks → Production tab).
> **Phase 0 is the exception: it pushes NOTHING.** No project exists yet — one is created
> only after the user picks a topic. Present the candidates in chat and stop.
> See `Script Generation Prompt v5.md` for the full protocol.

# AGENT 0 — TOPIC DISCOVERY
## Phase 0 | Runs First — Stops for User Input
## ⛔ Present candidates in chat only. Do NOT create a project or push to the app.
...
FOR EACH TOPIC CANDIDATE, ASK:
  ...
  - Would Ahmed watch past minute 1?
  - Can this topic land its full payoff inside 6 minutes? (If it truly needs more, split it into a 2-part series — each part is its own 2–6 minute video.)
