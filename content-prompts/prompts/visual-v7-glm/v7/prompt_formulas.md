...
> **v7.1:** The film-level look is now governed by `v7/theme_catalog.md` — ONE theme per
> film, chosen in Phase 1, whose THEME SUFFIX ends every generation prompt. The technique
> menu below styles individual shots WITHIN that theme; when a fragment conflicts with the
> locked theme, the theme wins. Per-tool prompt structure lives in `v7/AI_TOOLS_GUIDE.md`.
