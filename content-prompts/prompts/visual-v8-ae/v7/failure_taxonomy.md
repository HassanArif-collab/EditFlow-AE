...
| Highlight misalignment | Yellow sweep misses the cited phrase | Highlight position guessed instead of measured | Use the captured crop of the cited phrase to set highlight geometry | No rebuild; targeted repair |
| `VIBRATING_FRAME` | Frame shimmers, boils, or vibrates on hold | ANY flicker loop, per-frame noise reseed, infinite breathe/pulse, or sub-pixel scale wobble | Delete the oscillation; grain becomes one static fixed-seed overlay (0.03-0.05); holds are still with ≤0.5%/s one-direction drift | No rebuild; targeted repair |
