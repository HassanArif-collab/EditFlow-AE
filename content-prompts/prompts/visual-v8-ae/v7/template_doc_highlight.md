...
**Source:** user-captured screenshots dropped into the visuals folder (or document recreation)
...
Phase 5 (7.0s)  — Vignette subtly darkens edges. STATIC grain overlay (0.03-0.05, frame-locked).
Phase 6 (8.0s)  — Stat or callout reveal lands (if applicable — count-up from 0).
Phase 7 (10.0s) — Hold on evidence, STILL. One-direction drift ≤0.5%/s allowed. NO flicker, NO breathing — oscillation is banned (stability rules).
...
4. **Highlight bar:** A semi-transparent yellow/gold bar that paints across the cited sentence. Position comes from the captured crop of the cited phrase — never guessed by eye.
5. **DOF layer:** Non-hero text receives 2-4px blur. Hero text stays sharp.
6. **Grain layer:** ONE static film-grain overlay, opacity 0.03-0.05, coarse, fixed seed — never regenerated per frame, never over the hero text.
...
- Highlight bar position is a prop set from the captured crop of the cited phrase
