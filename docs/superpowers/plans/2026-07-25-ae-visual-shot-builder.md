...
Five builders, named for what they do rather than by letter:

| Builder | Who writes the AE code | Runs where |
|---|---|---|
...
All five take the **same** `shotlist.json` and write into the **same** AE project, each build landing as a new **version** of its shot (see "Versions, not overwrites"), so comparing them is a dropdown, not a separate system.

**Model note:** `nemotron-3-ultra:cloud` (550B, 55B active) is the intended code-writing model via Ollama. Verified reachable 2026-07-25 — the account currently returns *"weekly usage limit reached"*, so real runs need the paid tier. `nemotron-3-nano:4b` runs locally with no quota and is the free iteration model. Model per builder is a panel setting, recorded with every version.

**Tech Stack:** CEP panel (vanilla ES modules), ExtendScript ES3, node:test, the existing agent bridge for verification, Ollama via the backend's provider layer, and the Documentary Studio app's tunnel API for WEB-CODER.
...
Two extra columns exist only for the code-writing builders: **first-try rate** (compiled and ran with no retry — the honest measure of whether the environment brief works) and **ES3 failures** (how often the model reached for `const`/arrow/`toLocaleString`, the single most common mistake). GUIDED gets its own column instead: **fields you had to fix by hand** — because its job is saving you time, not beating anyone on fidelity.

**The control group is the point.** If BASELINE (no AI at all) ties the agent lanes, that's the answer — use the compiler and stop paying for tokens. I expect BASELINE to win on these three archetypes precisely *because* they're deterministic, and to lose the moment you add an archetype nobody wrote a builder for. That's the real question the bake-off answers: **where does the agent start earning its keep?**

**What I actually expect** (written down so the scoreboard can prove me wrong): BASELINE wins on these three archetypes because they're arithmetic. MENU is the dark horse — if composing primitives is enough, it's the cheapest thing that works. The two coders are the real experiment: a 550B model writing ES3 against a web model writing ES3, same prompt, same docs. And GUIDED isn't competing on quality at all — it's competing on *how much of your time it takes*, which no other builder measures.
...
(Next.js, port 3000, tunnel)               (FastAPI 8765 + AE panel)
  ...
  visual plans  ── shotsJson ──────────────► panel loads shotlist
                ── aeCode (WEB-CODER) ────────► panel loads generated builder
...
## Safety: WEB-CODER executes AI-written code
...
| `cep-panel-ae/client/src/builder-menu.js` | NEW. MENU: prompt + parse + validate for the local Ollama translator. |
| `cep-panel-ae/client/src/builder-coder.js` | NEW. WEB-CODER: load a pushed builder, run the static gate, hand it to AE. |
| `cep-panel-ae/client/src/bakeoff.js` | NEW. Runs all lanes over one shotlist, collects the scorecard. |
| `cep-panel-ae/extendscript/coder_loader.jsx` | NEW. Loads a gated Lane-A builder file inside AE and calls it. |
| `backend/routes/chat.py` | RESTORED from the EditFlowAI repo (Task 4.1) — MENU needs a completion endpoint. |
| `tests/builder-menu.test.js`, `tests/builder-coder-gate.test.js`, `tests/bakeoff.test.js` | NEW. Prompt/parse, the static gate, and scoring — all pure, node-tested. |
...
## Phase 4 — MENU: compose from primitives
...
- [ ] **Step 2: Trim it to what MENU needs.** Delete `POST /session/new` and any handler that imports a service this repo no longer has. Keep `POST /chat/message`. Verify with:
...
```python
"""MENU is useless if the prompt reaches the model altered — the whole
...
### Task 4.2: MENU — model composes primitives, never raw code

**Files:** Create `cep-panel-ae/client/src/builder-menu.js`, `tests/builder-menu.test.js`.
...
- [ ] **Step 4: Write the prompt builder** (same file) — it hands the model the shot, the archetype rules verbatim from v7, the primitive list, and demands JSON only. Keep it in one exported `buildPrompt(spec)` so WEB-CODER can reuse the identical text (fair comparison).

- [ ] **Step 5:** Tests PASS. **Commit** `feat(visuals): MENU — local model composes primitives, never raw code`.

## Phase 5 — LOCAL-CODER and WEB-CODER: models that write ExtendScript
...
### Task 5.1: The static gate (WEB-CODER only)

**Files:** Create `cep-panel-ae/client/src/builder-coder.js`, `tests/builder-coder-gate.test.js`.

- [ ] **Step 1: Write the failing tests** — the gate is the safety boundary, so it gets tested hardest:

```js
const OK = 'function ef_gen_shot01(comp, spec) { return true; }';
...
test('code touching the project or filesystem is refused', () => {
  for (const bad of [
    'function ef_gen_x(){ app.project.close(); }',
    'function ef_gen_x(){ new File("C:/x.txt").remove(); }',
    'function ef_gen_x(){ system.callSystem("cmd /c del *"); }',
    'function ef_gen_x(){ $.evalFile("other.jsx"); }',
    'function ef_gen_x(){ app.executeCommand(2); }',
  ]) {
...
test('functions outside the ef_gen_ namespace are refused', () => {
...
```js
/* Refused outright. WEB-CODER builds comps; it never touches projects, disk,
...
export function gateCode(src) {
  ...
  const stray = defined.filter((n) => !n.startsWith('ef_gen_'));
  if (stray.length) return { ok: false, reason: `namespace violation: ${stray.join(', ')}` };
  return { ok: true, reason: '', functions: defined };
}
```

- [ ] **Step 4:** Tests PASS. **Commit** `feat(visuals): WEB-CODER static gate — refuse unsafe generated code`.

### Task 5.2: Run a generated builder in AE

**Files:** Create `cep-panel-ae/extendscript/coder_loader.jsx`; modify `builder-coder.js`.
...
function ef_gen_run(jsonStr) {
    var started = false;
    try {
        var cfg = eval("(" + jsonStr + ")");
        if (!cfg.src || !cfg.fn || cfg.fn.indexOf("ef_gen_") !== 0) {
        ...
        app.beginUndoGroup("EditFlow: WEB-CODER " + cfg.fn);
        started = true;
        eval(cfg.src);                       // defines ef_gen_*
...
### Task 5.3: The code-writing instruction file
...
**Files:** Modify `cep-panel-ae/client/src/builder-coder.js`.
...
- [ ] **Step 2:** Log every attempt to `results.json` as `attempts: [{n, failure, fixed}]` so the scorecard can show *"WEB-CODER: 2 of 3 shots first-try, 1 after correction"* — first-try rate is the number that actually tells you whether the brief is working.
- [ ] **Step 3: Commit** `feat(visuals): WEB-CODER error round-trip with one correction attempt`.

## Phase 6 — GUIDED: you choose the treatment, the model fills it in

### Task 6.0: GUIDED — your taste, its typing

**Files:** Create `cep-panel-ae/client/src/builder-guided.js`, `tests/builder-guided.test.js`.

The inversion that matters: **you are the art director.** You pick the treatment — which title animation, which layout, which accent colour — and the model does the donkey work of filling it with real content: pulling the right number out of the shot, finding and placing the image, writing the label and caption text.

Nothing creative is delegated. Nothing tedious is manual.

- [ ] **Step 1: Write the failing tests** — the contract is the mirror of the code-writing builders: your choices are locked, the model may only supply content.

```js
test('the treatment you picked is never overridden by the model', () => {
  const chosen = { variant: 'slide_left', accentColor: '#b8860b', layout: 'centered' };
  const filled = G.applyFill(
    { id: 's3', archetype: 'SECTION_TITLE_CARD', treatment: chosen },
    { variant: 'scale_center', title: 'THE MONEY TRAIL', supporting: 'Act II' });
  assert.equal(filled.treatment.variant, 'slide_left', 'your choice wins');
  assert.equal(filled.title, 'THE MONEY TRAIL', 'model content accepted');
  assert.equal(filled.supporting, 'Act II');
});

test('content fields the archetype does not define are dropped', () => {
  const filled = G.applyFill(
    { id: 's1', archetype: 'STAT_COUNTER', treatment: { variant: 'centered' } },
    { title: 'Lost Revenue', value: 1500000000, nonsense: 'x', duration: 90 });
  assert.equal(filled.title, 'Lost Revenue');
  assert.equal(filled.value, 1500000000);
  assert.equal(filled.nonsense, undefined);
  assert.equal(filled.duration, undefined, 'timing is not the model\'s to set');
});

test('a value that contradicts the shotlist loses to the shotlist', () => {
  // the shotlist is the source of truth for data that came from your research
  const filled = G.applyFill(
    { id: 's1', archetype: 'STAT_COUNTER', value: 1500000000, treatment: {} },
    { value: 999 });
  assert.equal(filled.value, 1500000000);
});

test('an image suggestion must name a file, not describe one', () => {
  assert.equal(G.applyFill({ id: 's1', archetype: 'STAT_COUNTER', treatment: {} },
                           { bgSrc: 'port.jpg' }).bgSrc, 'port.jpg');
  assert.equal(G.applyFill({ id: 's1', archetype: 'STAT_COUNTER', treatment: {} },
                           { bgSrc: 'a photo of a busy port' }).bgSrc, undefined);
});
```

- [ ] **Step 2: Run** `node --test tests/builder-guided.test.js` → FAIL. **Step 3: Implement**

```js
/* Content the model MAY supply, per archetype. Everything else — your
   treatment, the timing, and any value already present in the shotlist —
   is off limits. This whitelist is why GUIDED cannot quietly change the
   number your research produced. */
export const FILLABLE = {
  SECTION_TITLE_CARD: ['title', 'supporting', 'bgSrc'],
  STAT_COUNTER: ['title', 'value', 'prefix', 'unit', 'bgSrc'],
  BAR_CHART: ['bars', 'caption', 'bgSrc'],
};

const FILENAME_RE = /^[\w.\- ]+\.(jpg|jpeg|png|mp4|mov)$/i;

export function applyFill(spec, fill) {
  const allowed = FILLABLE[spec.archetype] || [];
  const out = { ...spec };
  for (const k of Object.keys(fill || {})) {
    if (!allowed.includes(k)) continue;                    // not the model's field
    if (spec[k] !== undefined && spec[k] !== null && spec[k] !== '') continue;  // shotlist wins
    if (k === 'bgSrc' && !FILENAME_RE.test(String(fill[k]))) continue;  // a name, not a wish
    out[k] = fill[k];
  }
  return out;
}
...
- [ ] **Step 4: The treatment picker** (`visuals-view.js`) — per shot, a small set of choices drawn from the v7 approved lists: title variant (`slide up` / `scale from centre` / `slide from left` / `fade with rotation settle`), accent colour, and layout. Your pick is stored on the shot as `treatment` and is what the builder obeys.

- [ ] **Step 5: The fill call** — `requestFill(spec)` sends the shot, its `scriptLine`, the archetype's required content fields, and the list of files actually present in the assets folder (so `bgSrc` can only name something real). Response → `applyFill` → BASELINE builds it → lands as a new version.
...
test('scoreboard totals per lane and names a winner only on a real margin', () => {
  const board = B.scoreboard({
    baseline: [{ built: true, fidelity: 1, rules: 1, ms: 900 }],
    menu: [{ built: true, fidelity: 1, rules: 1, ms: 8000 }],
  });
  assert.equal(board.winner, 'baseline', 'tie on quality → faster lane wins');
  const tie = B.scoreboard({ baseline: [{ built: true, fidelity: 1, rules: 1, ms: 1000 }],
                             menu: [{ built: true, fidelity: 1, rules: 1, ms: 1050 }] });
...
{ "shotlist": "sample", "ranAt": "2026-07-25T10:00:00Z",
  "lanes": { "baseline": { "built": 3, "fidelity": 1.0, "rules": 1.0, "ms": 2700 },
             "menu": { "built": 3, "fidelity": 1.0, "rules": 0.67, "ms": 24000 },
             "webCoder": { "built": 2, "fidelity": 1.0, "rules": 1.0, "ms": 41000 } },
  "winner": "baseline",
  "frames": { "baseline": ["shot_01@1.5s.png", "..."] } }
```
...
- [ ] **Step 1:** Repeat Task 8.1's dump-and-render checks for **MENU** and **WEB-CODER** on the same 3 fixture shots, into the sandbox project (`EditFlow Bakeoff.aep`).
- [ ] **Step 2:** Confirm the WEB-CODER gate actually bites: hand `loadLaneABuilder` a builder containing `app.project.close()` and assert the panel refuses it and never reaches AE.
...
- [ ] **Step 2: Write `ae-visual-bakeoff.md`** — how to run all four lanes over one shotlist, what each scorecard column means, how to read `results.json`, how to accept a winning comp, and the safety rules for WEB-CODER (git-only delivery, the static gate, the sandbox project, the manual load click).
...
**WEB-CODER — v2 (see Phase D).** The code-writing lane is deferred until BASELINE + GUIDED are proven on one real video. Its plan text stays in this document so nothing is lost.
...
- **Resolved blocker:** the chat completion endpoint the repo cleanup removed is restored in Task 4.1 by copying it from EditFlowAI. Phases 1–3 don't depend on it, so BASELINE ships even if MENU stalls.
- **Risks:** (1) `ADBE Text Range Type2` = 1 for Characters is unverified in AE 2026 — the same enum risk the captions engine carries, and Task 8.1's frame render is what catches it; (2) `ADBE Vector Rect Position` anchoring for baseline bar growth needs the in-AE check in Task 8.1 Step 3; (3) fonts stay at the AE default in v1 — the Style Bible's typeface is a v2 concern; (4) WEB-CODER runs model-written code — mitigated by git-only delivery, the static gate (Task 5.1), a sandbox project, and a manual load click, and the gate's refusal is itself tested in Task 8.2.
- **Honest expectation:** BASELINE should win on these three archetypes — they're pure arithmetic, which is exactly where code beats a model. The bake-off's real value is finding the archetype where that stops being true, because that's the point where the agent starts paying for itself. If BASELINE wins everything, that is a result worth having, not a failure.
